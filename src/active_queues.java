...
@WebServlet("/active_queues")
public class active_queues extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
	        ...
	        //result set rx is to delete old expired queues
	        ResultSet rx=stmt.executeQuery("select q_id from active_q where valid_till < curdate();");
	        while(rx.next()) {String q_id=rx.getString("q_id");stmt.executeUpdate("delete from active_q where q_id="+q_id+";");stmt.executeUpdate("drop table queue_"+q_id+";");}
		...
		out.println( 
				...
				while(rs.next()) {
					...
					out.println("<tr class=\"table-dark\"> "+
							...
							"				</td>\r\n"+
							"				<td>" + 
							"				<form action=\"std_book_slot\"><input type=\"hidden\" name=\"q_id\" value="+q_id+"><button type=\"submit\" class=\"btn btn-outline-info\" "+ 
							"				 style=\"border-radius:20px;\">Book</button></form></td>" + 
							"				</tr>"); }
				out.println( 
				...
				"");
		       		...
		       		System.out.println("error in active_queues"+e.getMessage());
		       	}}else {
